...
import java.util.*;
import java.sql.*;
import sistemasdeventa.entiidadesdenegicios.*;
...
public class UsuarioDAL {
    ...
    public static int crear(Usuario pUsuario) throws Exception {
        ...
        if (existe == false) {
            try (Connection conn = ComunDB.obtenerConexion();) {
                sql = "INSERT INTO Usuario(IdRol,Nombre,Apellido,Login,Pass) VALUES(?,?,?,?,?,?,?)";
                try (PreparedStatement ps = ComunDB.createPreparedStatement(conn, sql);) {
    ...
    public static int modificar(Usuario pUsuario) throws Exception {
        ...
        if (existe == false) {
            try (Connection conn = ComunDB.obtenerConexion();) {                
                sql = "UPDATE Usuario SET IdRol=?, Nombre=?, Apellido=?, Login=?, WHERE Id=?";
                try (PreparedStatement ps = ComunDB.createPreparedStatement(conn, sql);) {
                    ps.setInt(1, pUsuario.getIdRol());
                    ps.setString(2, pUsuario.getNombre());  
                    ps.setString(3, pUsuario.getApellido());
                    ps.setString(4, pUsuario.getLogin());
                    ps.setInt(5, pUsuario.getId());
                    result = ps.executeUpdate();
                    ps.close();
                } catch (SQLException ex) {
                    throw ex;
